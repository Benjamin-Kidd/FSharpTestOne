...
let north exits = exits.North //({North = northExit}: Exits) = northExit
let south exits = exits.South //({South = southExit}: Exits) = southExit
let east exits = exits.East //({East = eastExit}: Exits) = eastExit
let west exits = exits.West //({West = westExit}: Exits) = westExit
...
//basicGameLoop.Post(UpdateState (move north))
//basicGameLoop.Post(UpdateState (move east))
//basicGameLoop.Post(EndGameLoop)
//